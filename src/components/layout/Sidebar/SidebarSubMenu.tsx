  ...
  return (
    <li className="relative text-center" key={route.name}>
      ...
      <ul
        ...
        className="space-y-2 overflow-hidden text-sm font-medium text-gray-500 rounded-md shadow-inner dark:text-gray-400 dark:bg-gray-900"
        aria-label="submenu"
      >
        {route.routes.map((r: any) => (
          <li
            className="pl-10 py-1 text-left transition-colors duration-150 hover:text-gray-200 dark:hover:text-gray-200"
            key={r.name}
          >
            <NavLink
              className={(navData) =>
